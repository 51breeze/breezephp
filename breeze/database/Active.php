...
abstract class Active extends Record implements IActive
{
    ...
    */
     ...
     */
    public function where( $column=null ,$value=null , $strainer=Strainer::EQUAL , $logical='AND' )
    {
        $s=& $this->getSql('where');
        if( empty($s) )
        {
            $s= new Strainer( $this );
            $this->setSql('where',$s,null,false,false );
        }
        if( empty($column) ) return $s;
        $s->setItem($column,$value,$strainer,$logical);
        return $this;
    }
