...
class Mysql extends Active implements IAdapter
{
    ...
    {
        $this->lastQuery=$sql;
        if( $this->dispatchDatabaseEvent( DatabaseEvent::BEFORE, $this->lastQuery ) )
        {
             $this->result=mysql_query( $sql , $this->link );
             $this->dispatchDatabaseEvent( DatabaseEvent::AFTER, $this->lastQuery , $this->result );
             if( $this->result === false )
                 throw new Error(  sprintf('%s:[%s]%s',Lang::info(2001),$this->getErrorInfo(), DEBUG===true ? "\r\nsql:".$sql : '' )   );
        }
        return $this;
    }
    ...
    public function execute( $sql='' )
    {
       ...
       if( $this->dispatchDatabaseEvent( DatabaseEvent::BEFORE, $this->lastQuery ) )
       {
           $resutl=mysql_query( $sql , $this->link );
           $this->dispatchDatabaseEvent( DatabaseEvent::AFTER, $this->lastQuery , $resutl );
           return ( $resutl===true );
       }
       return null;
    }
   ...
   /**
    * @see \breeze\interfaces\IAdapter::truncate()
    */
    public function truncate( $table='' )
    {
        return $this->execute( 'truncate ' .  $this->getTable( $table ) );
    }
    ...
    */
    ...
    public function fetch( $type=Database::FIELD , $rows=true )
    {
        ...
        {
        	Utils::message('没有可用的数据资源', Utils::DEBUG );
        }
        return $data;
    }
