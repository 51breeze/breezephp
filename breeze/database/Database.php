...
abstract class Database extends EventDispatcher implements IRecord,IAdapter,IActive
{
    ...
    protected $SQLSyntax=array(
            ...
            //清空表数据
            'clean'  => array('truncate', array('table') ),
    ...
    {
        $val=is_string($val) ? preg_replace('/^\s*(AND|OR|NOT)\s*$/i', '\\1', $val) : $default;
        return ' '.strtoupper( time($val) ).' ';
    }
     ...
     * @throws Error
     */
    public function type( $name , $length='', $decimals='', $formatType='' )
    {
        $args = func_get_args();
        ...
        {
            $formatType =  end( $args );
            $this->setSql('type', sprintf($name, implode(',', $args), $formatType ) );
            return $this;
        }
    ...
    protected function getColumnsInfo( $table, $full=true )
    {
        if( !isset($this->structure[ $table ]) )
        {
            ...
            $this->structure[ $table ]=$this->fetch();
        }
    ...
    protected function getMultiTablePrimary($table,$priority='PRI')
    {
        ...
        if( $priority === true )return $primary;
        if( !empty($primary) )
        {
    ...
    */
     ...
     */
    final protected function setSql($key='',$value='', $separate=null, $append=true ,$dispatch=true)
    { 
     ...
     * @private
     */
    final protected function merge( & $data=array(),$value )
    {
       if( !is_array($data) )
           $data=array();
     ...
     */
    final protected function & getSql( $key , $default='' )
    {
        $index= $this->isUnion===true  ? $this->index : 0 ;
        $data=$default;
        if( isset( $this->sql[$index] ) )
     ...
     * @private
     */
    final protected function hasSql( $key='' )
    {
        $index= $this->isUnion===true  ? $this->index : 0 ;
