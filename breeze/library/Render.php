 ...
 */
class Render extends EventDispatcher implements ISingleton
{
    /**
     * @public
     * 版本号
    ...
    {
        echo $this->fetch( $name ,$cache , $debug);
    }
     ...
     * @param $name
     */
    public function fetch( $name='', $cache=true, $debug=true )
    {
        ...
        //是否获取缓存内容
        $cacheEnable=$this->cacheEnable & $cache;
        $html=null;
        if( $cacheEnable )
        {
            //获取压缩方法名
        ...
        if( $html===null )
        {
            ...
            $debug===true && $html.= $this->debuginfo( $this->debuginfo );
    ...
    private function compileTemplate( $tpl_name , array $variable ,$cacheEnable=false,$debugEnable=false)
    {
        ...
        //导出变量到当前作用域
        empty( $variable ) || extract( $variable ,EXTR_IF_EXISTS OR EXTR_REFS );

        //判断此模板是否编译过
        if(  $data===null && file_exists( $compile_path ) )
        {
            //检查是否有更新
            ...
            {
                if( $event->result['md5'] !== md5_file( $tpl_path ) || self::VERSION !== $event->result['version'] )
                {
            ...
            if( $tpl_name=='head'){
              ...
              //  echo ob_get_clean();
               ...
               // echo file_get_contents( $compile_path );

                echo'========',$compile_path, $tpl_name;
               // exit;
            }

            if( $updated===false )
            {
               $error_handler===true && restore_error_handler();
                return;
               return ob_get_clean();
            }
        }
        ...
        //获取模板文件的内容
        if( $data===null && empty( $this->tpl_content ) )
        ...
        $data===null || $data=$this->internal( $tpl_path ,$tpl_name , $compile_path, $warning ).$data ;
        ...
        $write && file_put_contents( $compile_path, $data );
        eval('?>'.$data);
        $error_handler===true && restore_error_handler();
        return ob_get_clean();
    }
